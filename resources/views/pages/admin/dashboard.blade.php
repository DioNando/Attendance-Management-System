                        ...
                        @foreach($topEvents as $event)
                            <div class="flex h-full flex-1 flex-col items-center justify-end">
                                ...
                                <div class="w-full overflow-hidden text-center">
                                    <span class="block truncate text-xs font-medium text-gray-600 dark:text-gray-400" title="{{ $event->name }}">
                                        {{ \Illuminate\Support\Str::limit($event->name, 10) }}
                                    </span>
                                    <span class="text-xs font-bold text-gray-900 dark:text-white">{{ $event->guests_count }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="mt-8">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                @forelse($upcomingEventsList as $event)
                    <div class="overflow-hidden rounded-lg bg-white shadow-md transition-all hover:shadow-lg dark:bg-gray-800">
                        <div class="bg-blue-100 p-4 dark:bg-blue-900">
                            <div class="flex items-center justify-between">
                                ...
                                @endphp
                                @if($daysUntil > 0)
                                    <span class="rounded-full bg-blue-200 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-800 dark:text-blue-200">
                                        Dans {{ $daysUntil }} jour{{ $daysUntil > 1 ? 's' : '' }}
                                    </span>
                                @else
                                    <span class="rounded-full bg-green-200 px-2 py-1 text-xs font-medium text-green-800 dark:bg-green-800 dark:text-green-200">
                                        Aujourd'hui
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="p-4">
                            ...
                            <div class="mt-2 flex items-center text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <span>{{ $event->guests_count }} invité{{ $event->guests_count > 1 ? 's' : '' }}</span>
                            </div>
